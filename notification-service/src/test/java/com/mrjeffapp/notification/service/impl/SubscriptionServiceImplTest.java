...
public class SubscriptionServiceImplTest {
    ...
    private static final String BODY_TEMPLATE_SUBSCRIPTION = "template/mail/Subscription/Body";
    private static final String SUBJECT_TEMPLATE_SUBSCRIPTION = "template/mail/Subscription/Subject";
    ...
    public void setUp() {
        ...
        email = new Email(EMAIL_TYPE_CODE, EMAIL_DESTINATION, SUBJECT_TEMPLATE_SUBSCRIPTION, EMAIL_CONTENT);
    ...
    public void testValidBodyTemplatePath() {
        assertThat("the body template path must be", subscriptionService.BODY_TEMPLATE_SUBSCRIPTION, is(BODY_TEMPLATE_SUBSCRIPTION));
    }
    @Test
    public void testValidSubjectTemplatePath() {
        assertThat("the subject template path must be", subscriptionService.SUBJECT_TEMPLATE_SUBSCRIPTION, is(SUBJECT_TEMPLATE_SUBSCRIPTION));
    }
    ...
    public void testSendSubscriptionEmail() {
        ...
        when(templateService.renderBodyTemplate(eq(BODY_TEMPLATE_SUBSCRIPTION),eq(LANGUAGE_CODE), any(Map.class))).thenReturn(EMAIL_CONTENT);
        when(templateService.renderSubjectTemplate(eq(SUBJECT_TEMPLATE_SUBSCRIPTION),eq(LANGUAGE_CODE), any(Map.class))).thenReturn(SUBJECT_TEMPLATE_SUBSCRIPTION);
        ...
        subscriptionService.sendSubscriptionEmail(CUSTOMER_ID);
