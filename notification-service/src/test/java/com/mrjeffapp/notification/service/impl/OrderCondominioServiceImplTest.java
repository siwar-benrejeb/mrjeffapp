...
public class OrderCondominioServiceImplTest {
    private static final String ORDER_ID = "0000a2e7-228e-465c-8bc6-ddb1e7724e5c";
    private static final String PROJECTION = "full";
    private static final String BODY_TEMPLATE_ORDER_CONDOMINIO = "template/mail/OrderCondominio/Body";
    private static final String SUBJECT_TEMPLATE_ORDER_CONDOMINIO = "template/mail/OrderCondominio/Subject";
    ...
    @InjectMocks
    private OrderCondominioServiceImpl orderCondominioService;
    private Order order;
    ...
    public void setUp() throws ParseException {
        ...
        email = new Email(EMAIL_TYPE_CODE, EMAIL_DESTINATION, SUBJECT_TEMPLATE_ORDER_CONDOMINIO, EMAIL_CONTENT);
    ...
    public void testValidBodyTemplatePath() {
        assertThat("the body template path must be", orderCondominioService.BODY_TEMPLATE_ORDER_CONDOMINIO, is(BODY_TEMPLATE_ORDER_CONDOMINIO));


    }
    @Test
    public void testValidSubjectTemplatePath() {
        assertThat("the subject template path must be", orderCondominioService.SUBJECT_TEMPLATE_ORDER_CONDOMINIO, is(SUBJECT_TEMPLATE_ORDER_CONDOMINIO));

    }

    @Test
    public void testSendOrderCondominioEmail() {
        ...
        when(templateService.renderBodyTemplate(eq(BODY_TEMPLATE_ORDER_CONDOMINIO), eq(LANGUAGE_CODE), any(Map.class))).thenReturn(EMAIL_CONTENT);
        when(templateService.renderSubjectTemplate(eq(SUBJECT_TEMPLATE_ORDER_CONDOMINIO), eq(LANGUAGE_CODE), any(Map.class))).thenReturn(SUBJECT_TEMPLATE_ORDER_CONDOMINIO);
        ...
        orderCondominioService.sendOrderCondominioEmail(ORDER_ID);
