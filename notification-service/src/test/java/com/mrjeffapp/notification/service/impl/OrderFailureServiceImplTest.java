...
public class OrderFailureServiceImplTest {
    private static final String ORDER_ID = "0000a2e7-228e-465c-8bc6-ddb1e7724e5c";
    private static final String PROJECTION = "full";
    private static final String BODY_TEMPLATE_ORDER_FAILUURE = "template/mail/OrderFailure/Body";
    private static final String SUBJECT_TEMPLATE_ORDER_FAILUURE = "template/mail/OrderFailure/Subject";
    ...
    @InjectMocks
    private OrderFailureServiceImpl orderFailureService;
    private Order order;
    ...
    public void setUp() {
        ...
        email = new Email(EMAIL_TYPE_CODE, EMAIL_DESTINATION,SUBJECT_TEMPLATE_ORDER_FAILUURE, EMAIL_CONTENT);
    ...
    public void testValidBodyTemplatePath() {
        assertThat("the body template path must be", orderFailureService.BODY_TEMPLATE_ORDER_FAILUURE, is(BODY_TEMPLATE_ORDER_FAILUURE));


    }
    @Test
    public void testValidSubjectTemplatePath() {
        assertThat("the subject template path must be", orderFailureService.SUBJECT_TEMPLATE_ORDER_FAILUURE, is(SUBJECT_TEMPLATE_ORDER_FAILUURE));


    }

    @Test
    public void testSendFailedOrderEmail() {
        ...
        when(templateService.renderBodyTemplate(eq(BODY_TEMPLATE_ORDER_FAILUURE),eq(LANGUAGE_CODE), any(Map.class))).thenReturn(EMAIL_CONTENT);
        when(templateService.renderSubjectTemplate(eq(SUBJECT_TEMPLATE_ORDER_FAILUURE),eq(LANGUAGE_CODE), any(Map.class))).thenReturn(SUBJECT_TEMPLATE_ORDER_FAILUURE);
        ...
        orderFailureService.sendFailedOrderEmail(ORDER_ID);
